...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * 			즉, 서비스를 제공받는 고객
		 * 
		 * => Server는 자기 혼자 응답을 보낼 수 없음. 클라이언트의 요청이 들어와야만 응답을 보낼 수 있는 구조
		 * => 클라이언트는 서버에 요청을 보내기 위해서 서버의 IP주소와 Port번호를 알아야 함
		 * 
		 */
		
		// InetAddress : 네트쿼으 정보를 확인할 수 있는 클래스
		...
		try {
			...
			System.out.println(localHost); // PC이름 / IP 주소
			...
			System.out.println(googleHost); // 강사님과 google IP주소가 다르게 반환됨
	...
	// 결과물 source code : 문자열데이터, 자바 자바스크립트등의 언어로 작성
	// 프로그램 : 소스코드를 컴파일해서 작동 가능한 상태
	// 프로세스 : 프로그램이 메모리에 올라가 실행되고 있는 상태
	// thread : 실행 흐름의 단위, single multi
