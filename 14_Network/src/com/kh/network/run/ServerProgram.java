...
public class ServerProgram {
	...
	public static void main(String[] args) {
		 ...
		 * Socket
		 * - Process간의 통신을 담당
		 * - Socket객체를 이용해서 통신의 위한 기반스트림 InputStream / OutputStream을 제공받음
		 * - 기반스트리만으로는 제약이 있으니 보조스트림을 사용할 것
		 * 
		 * -- 소켓통신용 서버측 프로그램 작성 절차
		 * 
		 * 1) 포트번호 지정(서버측에서 몇 번 포트를 열어둘 것인지)
		 * 2) SeverSocket객체 생성 => 생성자 인자값으로 포트번호 전달(포트결합/Binding)
		 * 3) 대기상태
		...
		// 1) 포트번호 지정 // 사용 가능한 포트 1024번 ~ 49151번
		int portNumber = 1029;
		...
		try {
			...
			// socket객체를 이용해서 Client의 정보를 반환 받을 수 있음
