 ...
 * - (local) variable : 클래스 영역 내부에 특정한 구역에서 선언{}
 *                      => method, for, if 등등....
 * 
 * 1. 전역 변수의 '역할'을 하는 친구들
 * 
 * - 필드(멤버변수, 인스턴스 변수) : 
 *                        생성시점 => new 키워드 사용해서 해당 객체를 생성하는 순간 heap에 할당
 *                        소멸시점 => 객체가 소멸될 때 같이 소멸
 *                              => G.C가 일을 안하면 OOM(Out Of Memory) 발생
 * 
 * - 클래스 변수(static 변수, 정적 변수) : static이라는 예약어가 붙어있는 변수
 *                                 생성시점 : 프로그램 시작과 동시에 static영역에 올라감
 *                                 소멸시점 : 프로그램 종료되면 소멸
 * 
 * 2. 지역변수
 *        생성 시점 : 특정 영역{} 내부에서 선언되는 시점에 메모리영역(stack)에 할당
 *        소멸 시점 : 특정 영역{}이 종료될 때
 * 
 * 
 */
