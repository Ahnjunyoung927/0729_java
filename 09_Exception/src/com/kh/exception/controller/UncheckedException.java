...
public class UncheckedException {
	 ...
	 * - ArrayIndexOurOfBoundsException : 배열의 부적절한 인덱스로 접근했을때 발생하는 예외
	...
	// ArithmeticException
	public void method1() {
		// 사용자에게 두개의 정수값을 입력받아서 나눗셈을 한 뒤 결과를 출력
		...
		*/
		...
		// 방법 2. 예외처리를 진행
		// 예외처리 :
		// 예외 상황을 감지하고 예외상항이 발생 시
		// 프로그램이 비정상적인 종료가 되는 것을 방지하고 적절한 대응 조치를 취하는것
		/*
		 * try ~ catch문
		 * 
		 * [ 표현법 ]
		...
		try {
			System.out.println("연산결과 : " + (num1 / num2));
			System.out.println("올바른 정수 입력"); // 63행에서 예외발생시 64행은 생략하고 65행으로 진행됨
	...
	public void homeWork() {
		...
		while(true) {
			...
			System.out.println(menuNo + "번 메뉴를 선택하셨습니다.");
				...
				System.out.println("메뉴에 존재하는 정수를 입력해주세요");
				continue;
				// 예외처리시 catch문에 적어야하는 내용이 출력문은 아님
			}
		}
	}
	...
	public void method2() {
		...
		// ArithmeticException
		} catch(InputMismatchException e) { // 먼저 입력받은 값이 정수인지 판단
			System.out.println("숫자로 된 정수값이 아닙니다.");
		} catch(ArithmeticException e) { // 정수가 맞다면 0인지 판별
			System.out.println("0으로 나눌 수 없습니다.");
		}
	...
	public void method3() {
		...
		System.out.println("100번째 인덱스 값 : " + nums[100]);
		} catch(RuntimeException e) {
			e.printStackTrace(); // 얘는 꼭 개발단계에서만 사용
			System.out.println("세가지 예외처리를 한번에!");
		} 
